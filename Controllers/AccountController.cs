...
namespace LogBook.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult RoleAddToUser()
        {
            ...
            using (var context = new ApplicationDbContext())
            {
                users = context.Users.Select(u => u.UserName).ToList();
                roles = context.Roles.Select(r => r.Name).ToList();
            }
        ...
        public ActionResult GetRoles(string userName)
        {
            if (!string.IsNullOrWhiteSpace(userName))
            {
                ...
                using (var context = new ApplicationDbContext())
                {
                    ...
                    userRoles = context.Roles.Where(r => userRoleIds.Contains(r.Id)).Select(r => r.Name).ToList();
                    users = context.Users.Select(u => u.UserName).ToList();
                    roles = context.Roles.Select(r => r.Name).ToList();
                }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    return RedirectToAction("LogBook", "Home");
                }
        ...
        [AllowAnonymous]
        public ActionResult UnauthorizedUser(string returnUrl)
        {
        ...
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            loginInfo.Email = loginInfo.Login.ProviderKey + "@utah.gov";
            ...
            switch (result)
            {
                ...
                default:
                    ...
                    //    var uresult = await UserManager.CreateAsync(user);
                    //    if (uresult.Succeeded)
                    //    {
                    //        uresult = await UserManager.AddLoginAsync(user.Id, info.Login);
                    //        if (uresult.Succeeded)
                    //        {
                    //            await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
            ...
            string returnUrl)
        {
            if (String.IsNullOrEmpty(returnUrl) || returnUrl== "/")
                returnUrl = "/Account/UnauthorizedUser";
            if (User.Identity.IsAuthenticated)
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new ApplicationUser {UserName = model.Email, Email = model.Email};
                //user.Claims.Add(new IdentityUserClaim(ClaimTypes.Name, info));
                    ...
                    if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        await UserManager.AddClaimAsync(user.Id, new Claim("name", info.ExternalIdentity.Name));
                            await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                        return RedirectToLocal(returnUrl);
                    }
                }
...
//
        ...
        public ActionResult LogOff()
        {
            ...
            HttpContext.GetOwinContext()
                .Authentication
                .SignOut(CookieAuthenticationDefaults.AuthenticationType);
            return Redirect("https://login.utah.gov/user/portal");
        }
        [AllowAnonymous]
